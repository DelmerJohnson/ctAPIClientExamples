...
namespace client.fieldService.serviceDispatch.input.serviceCallEntry
{
    ...
    {
        private client.fieldService.serviceDispatch.input.serviceCallEntry.Form1 parentForm = null;

        public serviceCallsPopup(client.fieldService.serviceDispatch.input.serviceCallEntry.Form1 inParentForm, String inBatNbr)
        {
            InitializeComponent();
            parentForm = inParentForm;
            this.gvServiceCalls.AutoGenerateColumns = true;
            this.gvServiceCalls.DataSource = parentForm.mySCEObj.getServiceCalls(parentForm.tbServiceCallID.Text.Trim(), false);
        }
